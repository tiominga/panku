...
var cores=[cor2,cor2,cor2,cor2,cor2];
...
var arr_book=["0"];
var arr_abas=[];
var arr_abas_corrige=[];
var arr_allfiles=[];

var origem='';
var destino='';
var posicao_origem=0;
var posicao_destino=0;
...
{
var strbusca = eval('/'+esta+'/g');

 document.getElementById('auxiliar').value=document.getElementById('editor_main'+indexador).innerText;

 var conteudo_arquivo = document.getElementById('auxiliar').value;

 conteudo_arquivo=conteudo_arquivo.replace(strbusca,poresta);

 document.getElementById('auxiliar').value=conteudo_arquivo;
...
function procura(esta)
{

var strbusca = eval('/'+esta+'/g');

//coloca o que o usuário mandou como se fosse uma palavra reservada
arr_palavras[0]=esta;
...
var achei = conteudo_arquivo.match(/teste/g);
...
achei = achei.length;
 ...
 //document.getElementById('found').innerText=achei;
...
function abre_arquivo(arquivo)
{

	 var tot_abas = document.getElementById('direita').value;

		var pode = 1;

	 for (var i=0; i < tot_abas; i++)
	 {

	 		if (nome_arquivos[i]==arquivo && document.getElementById('aba_'+i).style.display !='none')
	 		{
	 ...
	 if (pode==1)
	 {
	 ftp_executa(arquivo,5);
	 }
	 else
	 {
	 	swd("It's alredy open");
	 }
         ...
         if (tipo==7 || tipo==8)
         {
            ...
            if (file_name) //se confirmou
            {
                ...
                if (file_name.length > 0) //se digitou alguma coisa
                {
						 ...
						 //verifica se o arquivo já existe, se existir não salva

						aba_ativa=window.parent.document.getElementById('aba_ativa').value;

						 var arquivos=arr_allfiles[aba_ativa];

						 var pode_salvar=1;

						 var arr_arquivos_verificar=arquivos.split(";");

						 var tot=arr_arquivos_verificar.length-1;

						 for (var i=0;i<tot;i++)
						 {

							if (arr_arquivos_verificar[i]==file_name)
							{
								pode_salvar=0;
						 ...
						 if (pode_salvar==0)
                   {
							  ...
							  if (confirm("Warning: the file "+file_name+" alredy exists. OVERWRITE???"))
							  {
      						pode_salvar=1;
  							  }
...
//muda a situação da div que mostra o iframe com resultado da query por exemplo. se vier 1 mostra se vier 0 esconde e 3 inverte (aquele +- na barra faz isso)
...
function editor_adiciona(conteudo_add,f9,nova,html)
{
          ...
          if (nova ==4)
          {
           ...
           //se é pra colocar em uma nova guia pra não perder a atual, aqui não joga o valor, só cria a nova aba mesmo
           if (nova==1)
           {

             var segura_aba=indexador;
             aba_cria();
             conexao_anterior();//pega os dados da conexão anterior
             verifica_resultado(1); //faz com que apareça a div que mostra o grid das querys
             turn_on(3);
          ...
          //se não perdesse o foco isso aqui funcionaria, deixarei aqui pois o futuro ...
          //   var segur = editor.innerText.substring(0, editor.selectionStart) +
          //   conteudo_add +
          //   editor.innerText.substring(editor.selectionEnd);

          conteudo_add=limpa(conteudo_add);

          document.getElementById('auxiliar').value=conteudo_add;

         // document.getElementById(editor_id).innerText=conteudo_add;

          if (nova != 3) //se o nova vem como 3 não é pra jogar no editor, só pra tirar o lixo tipo |@&%@ e jogar no auxiliar  (o formulario_ftp usa isso por exemplo)
          {
                  ...
                  if (f9==1)//se é para jogar e já sair consultando
                  {
                       ...
                       for (var i = 0;i < combo.length;i++)
                         ...
                         document.getElementById('tabelas_main'+indexador).innerHTML= document.getElementById('tabelas_main'+(indexador-1)).innerHTML;
                            document.getElementById('arquivos_main'+indexador).innerHTML= document.getElementById('arquivos_main'+(indexador-1)).innerHTML;
								...
								arr_allfiles[indexador]=arr_allfiles[indexador-1];
...
function mysql_script()
{
                                              ...
                                              var texto_aba='';
                                                ...
                                                document.getElementById('auxiliar').value=document.getElementById('editor_main'+indexador).innerText;
                                                consulta(document.getElementById('auxiliar').value,4);
...
function consulta(query,tipo) //tipo=0 não mostra nada, é query interna 1 mostra resultado 2 mostra só que fez 3 é o valor padrão que vai descobrir (o usuário que criou , não da pra saber) 4 é um dump a ser restaurado
{
       ...
       query=query.replace(/'/g, "|@&%@");
        ...
        var n_mostra=0;
         ...
         if (tipo==3)
         {
            ...
            //é inocente até provar o contrário (mostra até que se diga que não)
                  tipo=1;
         ...
         formulario.submit();//mandou para a formulario_sql_executa.php
...
//executa as alterações no grid, o arquivo formulario_sql_executa.php é quem chama esta função, procure lá por @|&@
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
   function clicou_aba(aba) //troca ao trocar de abas
   {
...
//percorre a matriz colocando os textos das abinhas como não clicadas
...
//o x tem que ficar da mesma cor (fechar)
                                         document.getElementById('texto2_'+(i+1)).style.color='#1D1B1C';
                                         document.getElementById('texto2_'+(i+1)).style.fontWeight='normal';
...
//o x tem que ficar da mesma cor (fechar)
                                         document.getElementById('texto2_'+(i+1)).style.color='#FFFFFF';
                                         document.getElementById('texto2_'+(i+1)).style.fontWeight='bolder';
...
//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    function fecha_aba(numero)
    {
     ...
     var posicao_fechando = 0;

//pega o número de abas abertas
     var nabas = arr_abas.length;
     nabas = nabas - 1;
...
//corrige o número de abas abertas
 ...
 //descobre a posição da aba fechada
 for(var i = 0; i < arr_abas.length; i++)
 {

      if (arr_abas[i]==aba_fechando)
      {
...
//se a aba que ele está fechando é a primeira e não tem nenhuma outra
    ...
    var pra_direita=0;
			...
			for (i=0; i < arr_abas.length;i++ )
			{
					...
					if (document.getElementById(arr_abas[i]).style.display != 'none')

                                   {
                                          document.getElementById(arr_abas[i]).style.left=pra_direita + '%';

                                           pra_direita = pra_direita+8;
  								...
  								/*ao iniciar o arrastar a aba*/
                        function drag_start(eu)
                        {
                           origem=eu;
                        ...
                        function drop(eu)
                        {
									...
									arr_abas[posicao_destino]=origem;
									arr_abas[posicao_origem]=destino;

									if (posicao_origem == posicao_destino)
									{

										for (var i=posicao_origem; i < posicao_destino;i++)
										{

										arr_abas[i]=arr_abas[i-1];
  ...
  function aba_cria()   //cria uma nova aba para o mesmo projeto que já está aberto. Pega os dados de conexao da ultima aba aberta
  {
          ...
          //   ccccc   rr   r      eeeeeee   aaaa      aaa      tt          eeeeee
                        ...
                        document.getElementById(id_aba).addEventListener("click", function() {
                         ...
                         document.getElementById(id_aba).addEventListener("dragstart", function() {

                                                       drag_start(this.id);

                                                              }, false);

                        document.getElementById(id_aba).addEventListener("drop", function() {

                                                       drop(this.id);

                                                              }, false);

                        document.getElementById(id_aba).addEventListener("dragover", function() {
...
//status bar 1
                         ...
                         ////////////////////////
                        //criaa uma status bar na div que fecha a barra de ferramentas da query
                        ////////////////////////
                        ...
                        //define a id dela
                        var id_st_fecha_ferramentas_query = 'st_fecha_ferramentas_query'+nabas;

                        //coloca um id na nova div
                          variavel_div.setAttribute( 'id',id_st_fecha_ferramentas_query);

                        //coloca um nome para o grupo, portanto, todos tem o mesmo nome, para depois poder percorrer
                        variavel_div.setAttribute( 'name','st_fecha_ferramentas_query');

                        //coloca atributos css na nova div   +cores[ncores]
                          variavel_div.setAttribute( 'style', 'position:absolute; white-space: nowrap; font-family:arial; font-size:12; cursor:pointer; display:block; overflow: auto; width:33%; height:98%; left:0%; top:0.5%; border-style:solid; border-width:0.0px;z-index:11');

                         /////////////coloca este botão para fechar efetivamente a div (evento onclick)
                         document.getElementById(id_st_fecha_ferramentas_query).addEventListener("click", function() {
                        ...
                        //define o pai do label que é o novo_id da camada que eu acabei de criar
                        pai =  document.getElementById(id_st_fecha_ferramentas_query);

                        //apresenta o pai para o filho
                        pai.appendChild(texto3);

                        var id_texto_status = 'status_'+nabas;

                        //coloca um id no novo label
                          texto3.setAttribute( 'id',id_texto_status);

                          texto3.setAttribute( 'style', 'font-family:verdana; color:#2B2C33; font-weight: bold;');

                        //coloca x como parametro para o label desta aba
                        document.getElementById(id_texto_status).innerText='...';
...
//statusbar 2
                         ...
                         ////////////////////////
                        //criaa uma status bar 2 na div que fecha a barra de ferramentas da query
                        ////////////////////////
                        ...
                        //define a id dela
                        var id_st2_fecha_ferramentas_query = 'st2_fecha_ferramentas_query'+nabas;

                        //coloca um id na nova div
                          variavel_div.setAttribute( 'id',id_st2_fecha_ferramentas_query);

                        //coloca um nome para o grupo, portanto, todos tem o mesmo nome, para depois poder percorrer
                        variavel_div.setAttribute( 'name','st2_fecha_ferramentas_query');

                        //coloca atributos css na nova div   +cores[ncores]
                          variavel_div.setAttribute( 'style', 'position:absolute; white-space: nowrap; font-family:arial; font-size:12; cursor:pointer; display:block; overflow: auto; width:33%; height:98%; left:33%; top:0.5%; border-style:solid; border-width:0.0px;z-index:11');

                         /////////////coloca este botão para fechar efetivamente a div (evento onclick)
                         document.getElementById(id_st2_fecha_ferramentas_query).addEventListener("click", function() {
                        ...
                        //define o pai do label que é o novo_id da camada que eu acabei de criar
                        pai =  document.getElementById(id_st2_fecha_ferramentas_query);

                        //apresenta o pai para o filho
                        pai.appendChild(texto3);

                        var id_texto_status = 'status2_'+nabas;
                        ...
                        //coloca um id no novo label
                          ...
                          var status_anterior='status2_'+(nabas-1);
                          document.getElementById(id_texto_status).innerText=document.getElementById(status_anterior).innerText;
...
//fim statusbar 2
                       ...
                       ////////////////////////
                        ...
                        //coloca atributos css na nova div   +cores[ncores]
                          ...
                          div_dados.setAttribute( 'style', 'position:absolute; white-space: nowrap; font-family:arial; font-size:12; color:rgba(0, 0, 255,0.4); display:none; overflow: auto; width:15%; height:45%; left:2%; top:10%; border-style:solid; border-width:0.3px;z-index:11');
