...
class DBtools
{
    ...
    private function setInformation($query, $db, $host, $port, $user, $password)
    {
        ...
        if ($type > 0 || $type == -3) { //user sends a query, -3 is an update
            ...
            if (!$res) { //error
                ...
                if ($type != 2 && $type != -3) { //it's not an update or show databases or describe
    ...
    private function SetQueryType($query) //called to connect.php
    {
        ...
        $array_a = array("show databases", "describe", "update ", "@php_property@;", "@php_get@;", "@php_post@;", "@php_set@;", "@new_panku_connection@", "@php_set_properties@");
        ...
        // > 0 then show results in the grid
        $arr_cod = array(2, 2, -3, 100, 101, 102, 103, 0, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115); //go to php_tools.php
